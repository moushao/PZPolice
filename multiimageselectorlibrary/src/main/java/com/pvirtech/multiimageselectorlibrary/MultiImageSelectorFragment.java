package com.pvirtech.multiimageselectorlibrary;
...
import com.pvirtech.multiimageselectorlibrary.adapter.FolderAdapter;
import com.pvirtech.multiimageselectorlibrary.adapter.ImageGridAdapter;
import com.pvirtech.multiimageselectorlibrary.bean.Folder;
import com.pvirtech.multiimageselectorlibrary.bean.Image;
import com.pvirtech.multiimageselectorlibrary.utils.FileUtils;
import com.pvirtech.multiimageselectorlibrary.utils.ScreenUtils;
        ...
        public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
            if (data != null) {
                if (data.getCount() > 0) {
                    ...
                    File cfile = getActivity().getExternalCacheDir();
                    if (cfile != null) {
                        String epath = cfile.getAbsolutePath();
                        // 图片目录
                        String imgPath = epath + "/img";
                        File imgPathFile = new File(imgPath);
                        if (imgPathFile.exists()&&imgPathFile.isDirectory()) {
                            List<Image> images2 = new ArrayList<>();
                            String[] sss = imgPathFile.list();
                            File[] files = imgPathFile.listFiles();

                            for (File image : files) {
                                if (!image.isDirectory()) {
                                    Image imageTemp = new Image(image.getAbsolutePath(), image.getName(), image.lastModified());
                                    images2.add(imageTemp);
                                }
                            }
                            Folder folder = new Folder();
                            folder.name = "工作拍摄";
                            folder.path = imgPath;
                            if (images2.size() > 0) {
                                folder.cover = images2.get(0);
                            }
                            folder.images = images2;
                            mResultFolder.add(folder);
                        }
                    }
                    if (!hasFolderGened) {
